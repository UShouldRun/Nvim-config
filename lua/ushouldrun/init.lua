...
local function change_wallpaper(image_path)
  local is_wsl = string.find(os.getenv("WSLENV") or "", "WSL")

  if is_wsl then
      -- print("Attempting to change Windows wallpaper from WSL...")

      image_path = "C:\\Users\\henri\\Pictures\\Wallpapers\\" .. image_path

      -- Convert the Linux path to a Windows path
      local wsl_to_win_command = string.format("wslpath -w '%s'", image_path)
      local handle = io.popen(wsl_to_win_command)
      local win_path = handle:read("*a"):gsub("[\n\r]", "")
      handle:close()

      if win_path and win_path ~= "" then
          -- Use the PowerShell command to change the wallpaper
          local powershell_command = string.format(
              "powershell.exe -c \"Set-ItemProperty -Path 'HKCU:\\Control Panel\\Desktop' -Name Wallpaper -Value '%s'; rundll32.exe user32.dll,UpdatePerUserSystemParameters,1,True\"",
              win_path
          )
          -- print("Running Windows command: " .. powershell_command)
          os.execute(powershell_command)
          -- print("Wallpaper change command sent to Windows.")
      else
          print("Failed to convert image path to Windows format.")
      end
